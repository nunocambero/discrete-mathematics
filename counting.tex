...
\begin{center}
    ...
    \hline
    \textbf{k labeled balls} & $k \leq n, V_n^k \text{ or } P(n,k)$ & $\displaystyle\frac{k!}{k_1! k_2! \dots k_n!}$ & \diagonalNA \\  
    \hline
    \textbf{k unlabeled} & $k \leq n, \displaystyle\binom{n}{k} = \frac{n!}{(n-k)!k!}$ & $\displaystyle CR_n^k$ & \diagonalNA \\  
    \hline
    \textbf{Unlimited balls} & \diagonalNA & \diagonalNA & $\displaystyle k^n$ \\  
    \hline
    \end{tabular}
\end{center}
...
\[
\text{Note that: } \left| \bigcup_{i=1}^{n} S_i \right| = \sum_{i=1}^{n} \left| S_i \right| - \sum_{i < j} \left| S_i \cap S_j \right| + \sum_{i < j < k} \left| S_i \cap S_j \cap S_k \right| - \dots + (-1)^{n-1} \left| S_1 \cap S_2 \cap \dots \cap S_n \right|
\]
